...
async def fetch_remote_time(target_node_id: str):
    ...
    print(f"DEBUG: Attempting to contact {target_node_id} at {url}")
...
async def start_periodic_sync(interval: int = 30):
    ...
    """
    while True:
        leader_name = consensus_service.leader_id
        ...
        if leader_name and leader_name != my_name:
            ...
            if result["status"] == "success":
                print(f"INFO: Sync successful. Target set to: {result['applied_target']:.6f}s")
            else:
                print(f"WARNING: Sync failed: {result['message']}")
        else:
            print(f"DEBUG: I am the leader ({my_name}) or no leader exists. Skipping sync.")
...
async def start_clock_slew():
    ...
    Prevents sudden time jumps in the distributed system."""
    global drift_offset, target_offset
    while True:
